//https://blog.csdn.net/qq_28580731/article/details/79891249?depth_1-utm_source=distribute.pc_relevant.none-task&utm_source=distribute.pc_relevant.none-task

Shader "N/special/3d/toon/NRP2ToonTest1"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				//采样的图的g通道乘以顶点r值
				float vrCr = lightMapColor.g;//; * i.color.r;//NTEMP	去掉骨骼颜色
 
 
				//阈值，关于第二层暗面的颜色，如果lColorG = 0,就是用第二层暗面颜色，否则使用第一层暗面颜色
				float lColorG = max(floor((vrCr + i.color1.x)* 0.5 + (-_SecondShadow) + 1.0), 0.0);
				...
				float lColorGx = max(floor( /**(-i.color.r* NTEMP 去骨骼颜色  **/ lightMapColor.g + 1.5), 0.0);
				float2 tempXY = vrCr.rr * float2(1.20000005, 1.25) + float2(-0.100000001, -0.125);
				vrCr = (lColorGx != 0) ? tempXY.y : tempXY.x;
				vrCr = max(floor((vrCr + i.color1.x) * 0.5 + (-_FirstShadowArea) + 1.0), 0);
				lColorGx = int(vrCr);
				firstShadowColor.rgb = (lColorGx != 0) ? mainColor.rgb : firstShadowColor.rgb;
 
 
				vrCr = lightMapColor.g;// * i.color.r; NTEMP 去掉骨骼颜色
