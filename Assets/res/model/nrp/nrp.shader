...
////// Lighting:
                float4 SpeData = tex2D(_SpeShaTex,TRANSFORM_TEX(i.uv0, _SpeShaTex));
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float3 node_1398 = saturate(( _MainTex_var.rgb > 0.5 ? (1.0-(1.0-2.0*(_MainTex_var.rgb-0.5))*(1.0-SpeData.g)) : (2.0*_MainTex_var.rgb*SpeData.g) ));
